...
# @Last Modified time: 2017-06-13 21:07:41
...
class CodeGenerator:
    ...
    def generate_variable(self, end, offset=0):
        ...
        if self.l_type == self.JAVA:
            return self.__gen_java_static(end, offset)
    ...
    def generate_function(self):
        ...
        if self.l_type == 'java':
            return self.__gen_java_func()
        # if self.l_type == 'python':
        #     return self.__gen_py_func()
        raise TypeError('Unsupported l_type: {0}'.format(self.l_type))

    def __gen_java_func(self):
        ...
        return "".join([func_define, func_body])
    ...
    def __body(self, printer):
        ...
        adder = "".join([adder_template.format(num=i, param_name=self.param_name_arr[i]) for i in range(len(self.param_name_arr))])
        limiter = limiter_template.format(adder=adder)
        walker = "_walker++;"
        returner_template = "return _{origin_func_name}({origin_params});"
        returner = returner_template.format(origin_func_name=self.func_name, origin_params=",".join(self.param_name_arr))
        return body_template.format(limiter=limiter,
                                    walker=walker,
                                    printer=printer,
                                    returner=returner)
    ...
    def __printer_body(self, formatter):
        print_body_template = "for (int i = 0; i < _walker - _home; i++) {{{formatter}}};return new {code_stopper};"
        code_stopper = re.sub('\[\]', '[0]', str(self.output_tree.get_root_child()[0]))
    ...
    def __sub_printer(self, recur_printer):
        ...
        recur_printer_template = "for ({sub_type} {pre_fix}p: _{num}Param.get(i)) {{{recur_printer}}}{separator}"
        code_arr = []
        for num in range(len(self.param_name_arr)):
            _node = self.input_tree.get_root_child()[num]
            if _node.get_type() not in ['List', '[]']:
                recur_printer_template = ''.join(['System.out.print("||-");',
                                                    'System.out.println(_{num}Param.get(i));',
                                                    'System.out.print("-||");', 
                                                    '{separator}'])
    ...
    def __java_recursive_printer(self, recur_printer_template, param_node, pre_fix):
        if param_node.get_type() == 'List' or param_node.get_type() == '[]':
            return recur_printer_template.format(sub_type=str(param_node.get_child_type()[0]),
                                                 ...
                                                 sub_printer=self.__java_recursive_printer(recur_printer_template,
                                                                                           ...
                                                                                           pre_fix + '_'))
        return 'System.out.print("||-");System.out.print({0}p);System.out.print("-||");'.format(pre_fix)

    def __gen_java_static(self, end, offset):
        static_counter = "static int _home = {0};static int _walker = 0;static int _office = {1};".format(offset, end)
        static_cache_template = "static List<{param_type}> _{param_num}Param = new ArrayList<{param_type}>();"
        static_statement_arr = [static_counter]

        for i, child in enumerate(self.input_tree.get_root_child()):
            static_statement_arr.append(static_cache_template.format(param_type=str(child), param_num=str(i)))
